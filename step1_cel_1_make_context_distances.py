...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 18 11:23:28 2023
...
####2)normalized expression tables (i) df_prot_exp_n.pkl for C.elegans 
####3)Delaunay distances at each time point in df_dist_Dln folder
####OUTPUTS:
####folder context_dist with a dataframe per time point with all the context distances
...
df_exp=pd.read_pickle(path_variables+'df_prot_exp_n.pkl') ####change if toy model
...
for t in range(13,191):
    ...
    for ic1 in range(np.shape(this_time_cells)[0]):
        ...
        for ic2 in range(ic1,np.shape(this_time_cells)[0]):
            ...
            df_n[c1][c2]=r
    df_n.to_pickle(path_save+'context_dist/'+'df_context_dist_t'+str(t)+'.pkl')
...
for t in range(13,191):
      ...
      df_nei.to_pickle(path_save+'context_dist/'+'df_context_dist_t'+str(t)+'.pkl')