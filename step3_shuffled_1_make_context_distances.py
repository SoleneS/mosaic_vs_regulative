...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 17:47:28 2024
...
####2)normalized expression tables (i) df_prot_exp_n_random.pkl for the random model 
...
df_exp=pd.read_pickle(path_variables+'df_prot_exp_n_random.pkl') ####change if toy model
...
for t in range(13,191):
    ...
    for ic1 in range(np.shape(this_time_cells)[0]):
        ...
        for ic2 in range(ic1,np.shape(this_time_cells)[0]):
            ...
            df_n[c1][c2]=r
    df_n.to_pickle(path_save+'context_dist_random/'+'df_context_dist_random_t'+str(t)+'.pkl')
...
for t in range(13,191):
      ...
      df_nei.to_pickle(path_save+'context_dist_random/'+'df_context_dist_random_t'+str(t)+'.pkl')