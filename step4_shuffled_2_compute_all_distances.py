...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 17:49:02 2024
...
####2)protein expression distances df_exp_dist_random.pkl
...
####6) previously computed context distances context_dist_random/'+'df_context_dist_random_t'+str(t)+'.pkl'
####OUTPUT
####1) table with all distances for all pairs in a given tissue 'df_all_dist_random_'+my_tissue+'.pkl'
...
df_r=pd.read_pickle(path_variables+'df_exp_dist_random.pkl') ####change if toy model
...
for my_tissue in all_tissues:
    df_bundles=pd.read_pickle(path_variables+'bundles_random/'+'df_bundles_'+str(my_tissue)+'.pkl')
    ...
    for t in range(13,191):
        ...
        df_nei=pd.read_pickle(path_save+'context_dist_random/'+'df_context_dist_random_t'+str(t)+'.pkl')
    ...
    df_all_dist.to_pickle(path_save+'df_all_dist_random_'+str(my_tissue)+'.pkl')
