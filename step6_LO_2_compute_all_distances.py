...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 17:50:54 2024
...
####2)protein expression distances df_exp_dist_LO.pkl
...
####6) previously computed context distances context_dist_LO/'+'df_context_dist_LO_t'+str(t)+'.pkl'
####OUTPUT
####1) table with all distances for all pairs in a given tissue 'df_all_dist_LO_'+my_tissue+'.pkl'
...
df_r=pd.read_pickle(path_variables+'df_exp_dist_LO.pkl') ####change if toy model
...
for my_tissue in all_tissues:
    df_bundles=pd.read_pickle(path_variables+'bundles_LO/'+'df_bundles_'+str(my_tissue)+'.pkl')
    ...
    df_all_dist.to_pickle(path_save+'df_all_dist_LO_'+str(my_tissue)+'.pkl')
